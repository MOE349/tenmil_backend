...
class WorkOrderPartBaseView(BaseAPIView):
    ...
    def cleanup_stuck_records(self, data, params, pk=None, *args, **kwargs):
        """
        Clean up stuck planning records for a WorkOrderPart.
        This consolidates multiple stuck planning records into a single record.
        
        URL: POST /parts/work_order_part/{pk}/cleanup_stuck_records/
        """
        try:
            from parts.services import WorkOrderPartRequestWorkflowService
            
            # Get the WorkOrderPart
            work_order_part = self.get_object(pk)
            
            # Get user from params
            user = params.get('user')
            
            # Cleanup stuck records
            result = WorkOrderPartRequestWorkflowService.cleanup_stuck_planning_records(
                wop_id=str(work_order_part.id),
                performed_by=user
            )
            
            return self.format_response(data=result, status_code=200)
            
        except Exception as e:
            return self.handle_exception(e)
    
    def get_stuck_records_info(self, data, params, pk=None, *args, **kwargs):
        """
        Get information about stuck planning records for a WorkOrderPart.
        This is a read-only operation to inspect stuck records.
        
        URL: GET /parts/work_order_part/{pk}/stuck_records_info/
        """
        try:
            # Get the WorkOrderPart
            work_order_part = self.get_object(pk)
            
            # Get stuck planning records
            stuck_records = work_order_part.get_stuck_planning_records()
            
            # Calculate total qty_needed
            ...
            result = {
                'wop_id': str(work_order_part.id),
                'work_order_code': work_order_part.work_order.code,
                'part_number': work_order_part.part.part_number,
                'stuck_records_count': stuck_records.count(),
                'total_qty_needed': total_qty_needed,
                'stuck_records': stuck_records_data,
                'has_active_workflow_flags': work_order_part.has_active_workflow_flags()
            }
    ...
    def cleanup_useless_drafts(self, data, params, pk=None, *args, **kwargs):
        """
        Clean up useless draft WOPR records for a WorkOrderPart.
        This removes records with all zeros and no active flags.
        
        URL: POST /parts/work_order_part/{pk}/cleanup_useless_drafts/
        """
        try:
            # Get the WorkOrderPart
            work_order_part = self.get_object(pk)
            ...
            # Cleanup useless draft records
            result = work_order_part.cleanup_useless_draft_records()
            
            # Add user context to result
            result.update({
                'success': True,
                'message': f"Cleaned up {result['deleted_count']} useless draft records",
